...
--@Descripcion: Comprobacion de consultas para comprobar liga de s1 a s2
...
begin
  ...
  from (
    ...
    union all
    select pais_id
    from f_aegg_pais_2@aeggbdd_s2.fi.unam
  ) q1;
  ...
  from (
    ...
    union all
    select oficina_id
    from f_aegg_oficina_2@aeggbdd_s2.fi.unam
  ) q2;
  ...
  from (
    select proyecto_id
    from f_aegg_proyecto_1
    union all
    select proyecto_id
    from f_aegg_proyecto_2@aeggbdd_s2.fi.unam
    union all
    select proyecto_id
    from f_aegg_proyecto_3@aeggbdd_s2.fi.unam
  ) q3;
  ...
  from (
    ...
    union all 
    select num_pdf
    from f_aegg_proyecto_pdf_2@aeggbdd_s2.fi.unam
  ) q4;
  ...
  from (
    ...
    from f_aegg_empleado_1@aeggbdd_s2.fi.unam e1
    join (
      select empleado_id
      from f_aegg_empleado_2
      union all 
      select empleado_id
      from f_aegg_empleado_3@aeggbdd_s2.fi.unam
    ) e2
  ...
  from (
    ...
    from f_aegg_pago_empleado_1
    union all
    select pago_empleado_id
    from f_aegg_pago_empleado_2@aeggbdd_s2.fi.unam
  ) q6;
